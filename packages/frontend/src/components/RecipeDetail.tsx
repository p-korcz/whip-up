...
export function RecipeDetail({ recipe, searchedIngredients, onBack }: RecipeDetailProps) {
  const { t } = useTranslation();
  const lowerSearched = searchedIngredients.map((i) => i.toLowerCase());
    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        ...
        <section className={styles.section} aria-labelledby="steps-heading">
          <h2 id="steps-heading" className={styles.sectionTitle}>{t('recipe.steps')}</h2>
      ...
      <footer className={styles.footer}>
        <a href={recipe.directUrl} target="_blank" rel="noopener noreferrer" className={styles.sourceLink}>
          {t('recipe.viewSource')}
        ...
        <span className={styles.sourceLabel}>{t('recipe.source')}: {recipe.sourceUrl}</span>
      </footer>
    </div>
  );
}
